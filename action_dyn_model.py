...
class PairwiseInteract(nn.Module):
    def __init__(self,
        ...
        for actee in self.actees:
            self.action_force_modules[actee] = MLP(action_force_layer_sizes)
            self.add_module('action_force_{}'.format(actee), self.action_force_modules[actee])
        for actee in self.actees:
        ...
        for (i, c1) in enumerate(self.actees):
            for c2 in self.actees[i:]:
                self.cost_modules[(c1, c2)] = MLP(cost_layer_sizes)
                self.add_module('cost_({},{})'.format(c1, c2), self.cost_modules[(c1, c2)])

    def forward(self, obj_locs, prev_obj_locs, action):
        ...
        for (actee, actee_objs) in zip(self.actees, obj_locs):
            forces[c] += self.action_force_modules[actee](action).reshape(1, -1)
        for (c, (actee, actee_objs)) in enumerate(zip(self.actees, obj_locs)):
        ...
        cost = torch.zeros(1, dtype=torch.float32)
        for (i, (c1, c1_objs)) in enumerate(zip(self.actees, obj_locs)):
            for (c2, c2_objs) in zip(self.actees[i:], obj_locs[i:]):
                combs = torch.stack([
                    c1_objs.reshape(1, -1, self.state_dim).repeat(len(c2_objs), 1, 1).reshape(-1, self.state_dim),
                    c2_objs.reshape(-1, 1, self.state_dim).repeat(1, len(c1_objs), 1).reshape(-1, self.state_dim),
                ], dim=1)
                combs = combs[:,0,:] - combs[:,1,:]
                cost += torch.sum(
                    self.cost_modules[(c1, c2)](combs),
                    dim=0,
                )
        return preds, cost
